...
{
    Tochka* A = new Tochka(a, SH(a));
    Tochka* B = new Tochka(b, SH(b));
    ...
    while (abs(pred_i_sled_shag.second - pred_i_sled_shag.first) > epsilon)
    {
        ...
        interval.Add(new Tochka(pred_i_sled_shag.second, SH(pred_i_sled_shag.second)));
    }
    return min_xy;
}
...
int main()
{
    double extrX = Algoritm(0, 10, 0.05, 4).first;
    double extrY = Algoritm(0, 10, 0.05, 4).second;
